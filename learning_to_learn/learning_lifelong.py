...
def generate_data(cluster_center, n_class, n_data_per_batch=100):
...
def train(model, cluster_center, n_epoch=20):
    optimizer = optim.Adam(model.parameters(), lr=0.01)
    for n_class in range(cluster_center.shape[0]):
        for epoch in range(n_epoch * n_class):
            batch = generate_data(cluster_center, n_class+1)
...
def validate(model, cluster_center, n_epoch=100):
    val_accuracy = []
    for epoch in range(n_epoch):
        batch = generate_data(cluster_center, cluster_center.shape[0])
        x, y = Variable(torch.from_numpy(batch[0])).float(), Variable(torch.from_numpy(batch[1]))
        output = model(x)
