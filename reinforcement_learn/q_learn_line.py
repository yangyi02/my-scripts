...
def show_state(x, y, angle):
    ...
    plt.plot([-5, 5], [1, 1], linestyle='-', color='b')
    plt.plot([-5, -5], [-1, 1], linestyle='-', color='b')
    plt.plot([5, 5], [-1, 1], linestyle='--', color='g')
...
def q_learn():
    ...
    for i in range(train_iter):
        ...
        logging.info('iteration: %d, exploration rate: %.4f, cumulative reward: %.4f, '
                     'average loss: %.4f',
                     i, exploration_rate[i], cumulative_reward, average_loss)
    plt.ioff()
    plt.show()
    return weight
...
if __name__ == '__main__':
    # test_functions()
    w = q_learn()
    # w = np.array([[-0.79600447, -0.48741915],
    #  [0.282672, -0.33031299],
    #  [0.83422109,  1.00890489],
    # [-1.70931452, -1.91985426]])
    # print w
    # q_test(w)
    # show_q_value(w)